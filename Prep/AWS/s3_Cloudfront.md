...
**Amazon S3 (Simple Storage Service)** is a fully managed, **object storage** service by AWS that allows you to **store and retrieve any amount of data** — images, videos, backups, logs, or big data — from anywhere.
It’s designed for **scalability, durability (99.999999999%)**, and **availability**, commonly used for **static website hosting**, **data lake storage**, and **application backups**.
...
| **Feature**                        | **Description**                                                               |
| ---------------------------------- | ----------------------------------------------------------------------------- |
| **Durability**                     | 99.999999999% (11 nines) – stored redundantly across AZs                      |
...
### 📋 Bucket-Level Configurable Features
...
However, a **single PUT upload request** (non-multipart) is limited to **5 gigabytes (GB)** — for larger objects, you must use **multipart upload**.
...
Each class is optimized for a specific use case — from **frequent access (Standard)** to **archival cold storage (Glacier)**.
...
| ----------------------------------------- | -------------- | ---------------- | ---------------------------------------------------------- | ------------------ | -------------------------------------- |
| 🟢 **S3 Standard**                        | 99.999999999%  | 99.99%           | Frequently accessed data                                   | Immediate          | 💰 High                                |
| 🟡 **S3 Intelligent-Tiering**             | 99.999999999%  | 99.9%–99.99%     | Automatically moves data between frequent/infrequent tiers | Immediate          | 💸 Variable                            |
| 🔵 **S3 Standard-IA (Infrequent Access)** | 99.999999999%  | 99.9%            | Long-lived but infrequently accessed data                  | Immediate          | 💸 Lower storage, small retrieval cost |
...
### 🧩 Example: Terraform – Lifecycle Transition Policy
...
### 🧩 Example: Change Storage Class via AWS CLI
...
aws s3 cp app.conf s3://my-versioned-bucket/   # Upload again → new VersionId
```
...
| **State**     | **Description**                                                                      |
| ------------- | ------------------------------------------------------------------------------------ |
| `Enabled`     | Versioning fully active — new uploads create unique versions.                        |
| `Suspended`   | Keeps existing versions but no new versions are created (latest overwrite replaces). |
| `Unversioned` | Default for new buckets — versioning not yet enabled.                                |
...
It helps you **control storage costs** and **data retention** by transitioning objects between **storage classes** (e.g., Standard → Glacier) or **deleting old versions** after a set time.
...
### 🧩 Example: Common Lifecycle Rules
...
| -------------------------------- | ------------------------------------------- | -------------------- |
| `Transition`                     | Move data to lower-cost class (IA, Glacier) | 30–180 days          |
| `Expiration`                     | Delete old or temporary files               | 30–365 days          |
...
By default, **Amazon S3 buckets are private** — only the owner (and IAM-authorized users) can access objects.
...
1. Go to **S3 → Buckets → your-bucket-name**
...
Essentially — **you sign an S3 request with your credentials**, and others can use that URL within the specified time.
...
➡️ Anyone with this URL can download the file for **1 hour**.
...
➡️ The receiver can upload directly via:
...
Amazon S3 ensures **extremely high data durability** — **99.999999999% (11 nines)** — by automatically **replicating your data across multiple devices and Availability Zones (AZs)** within an AWS Region.
...
| **Durability SLA**                 | Amazon’s design target: losing a single object once every **10 million years per 10,000 objects**.                                        |
...
Your Data
   │
   ├── Stored in S3 Bucket (e.g., ap-south-1)
   │     ├── Copy 1 → AZ-A (Primary)
   │     ├── Copy 2 → AZ-B (Secondary)
   │     └── Copy 3 → AZ-C (Tertiary)
   │
   └── Background integrity checks + automatic healing
```
...
**Amazon S3 (Simple Storage Service)** is a highly durable, scalable, and cost-effective **object storage** service used across nearly every domain — from web hosting to analytics.
...
| 💾 **Application Data Storage**   | Store app logs, exports, configs | Used by web and mobile apps for file uploads, logs, backups, and exports; integrates easily via pre-signed URLs.                            |
...
Amazon S3 can **directly host static websites** — HTML, CSS, JavaScript, and images — without any backend servers.
This is ideal for **personal blogs, documentation, landing pages, and SPAs** (Single Page Applications).
S3 provides **scalable, serverless, and low-cost hosting** — optionally accelerated with **CloudFront CDN**.

---

### ⚙️ Purpose / How It Works

S3’s **Static Website Hosting** mode serves your bucket’s content via an **HTTP endpoint** (no HTTPS).
You simply:

1. Create an S3 bucket with your domain name.
2. Upload static files (HTML, CSS, JS, images).
3. Enable “Static website hosting” in the bucket settings.
...
### 🧩 Optional: Add Custom Domain + HTTPS (via CloudFront)
...
1️⃣ Create a public bucket → 2️⃣ Upload files → 3️⃣ Enable static website hosting → 4️⃣ (Optional) front it with CloudFront for HTTPS.
...
Securing S3 means controlling **who can access your data**, **how it’s encrypted**, and **how access is monitored** — using AWS’s **defense-in-depth model**: IAM, encryption, network controls, and auditing.
...
### 🧩 2️⃣ Use IAM Policies (Principle of Least Privilege)

Grant **users and roles** only the permissions they need — no wildcards (`*`).
...
✅ **Tip:** Assign S3 permissions via **IAM roles** (for EC2, ECS, or Lambda), not access keys.

---

### 🧩 3️⃣ Use Bucket Policies Carefully

Use **resource-based bucket policies** only when necessary — for specific accounts or services (e.g., CloudFront OAC, AWS Logs).
...
✅ **Best practice:** Use **SSE-KMS** for sensitive workloads.

---

### 🧩 5️⃣ Enable Versioning & MFA Delete
...
### 🧩 6️⃣ Restrict Access by VPC or PrivateLink
...
✅ This ensures only traffic from your **private VPC endpoint** can reach the bucket.
...
### 🧩 8️⃣ Use Object Lock for Compliance
...
2️⃣ **Use IAM + bucket policies** with least privilege,
3️⃣ **Encrypt all data (SSE-KMS)**,
4️⃣ **Enable versioning and MFA delete**,
5️⃣ **Restrict via VPC endpoints**,
...
**Amazon S3 Object Lock** is a **data protection and compliance** feature that lets you store objects in **Write-Once-Read-Many (WORM)** mode — meaning once written, they **cannot be modified or deleted** for a defined retention period.
...
### 🧩 Example: Enable Object Lock on a Bucket
...
### 🧩 Example: Apply Retention to an Object
...
It enforces **WORM protection** to prevent any changes or deletions for a defined retention period — ensuring compliance, audit integrity, and protection against ransomware or insider threats.
...
**Server-Side Encryption (SSE)** in **Amazon S3** automatically **encrypts your data at rest** — meaning AWS encrypts your object **before saving it to disk** and decrypts it **when you access it**, without requiring manual handling of keys in your application.
...
### 🧩 Example 1: SSE-S3 (Default Encryption)

```bash
aws s3 cp data.csv s3://my-bucket/ --sse AES256
```

✅ AWS handles key creation, rotation, and protection.

---

### 🧩 Example 2: SSE-KMS (Customer Managed Key)
...
✅ Keys managed in **AWS Key Management Service (KMS)** — full control and audit via CloudTrail.

---

### 🧩 Example 3: SSE-C (Customer Provided Key)
...
✅ AWS uses your key only during encryption/decryption — never stores it.

---

### 📋 Default Bucket-Level Encryption (Recommended)
...
✅ **SSE-S3** = simple,
✅ **SSE-KMS** = controlled & auditable,
✅ **SSE-C** = fully customer-managed.
...
Both **SSE-S3** and **SSE-KMS** are forms of **Server-Side Encryption (SSE)** in Amazon S3 — meaning AWS encrypts your objects **before storing them** and **decrypts automatically** when accessed.
...
| ------------------------ | -------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| **Key Management**       | Keys managed entirely by AWS S3.                   | Keys managed by **AWS KMS (Key Management Service)** — you can create, rotate, and manage them. |
...
| **Access Control**       | Basic — bucket/object-level IAM controls only.     | Fine-grained — control key usage via **KMS key policies + IAM policies**.                       |
| **Auditability**         | No detailed audit of key usage.                    | All key operations logged in **AWS CloudTrail**.                                                |
| **Performance**          | Slightly faster (no KMS API call).                 | Slightly slower — involves KMS API call for key decryption.                                     |
...
✅ S3 automatically encrypts with an **AWS-managed AES-256 key**.
...
✅ Uses a **KMS CMK** — gives control over access, audit, and rotation.
...
**Amazon S3 Event Notifications** allow you to **automatically trigger actions** whenever specific events occur in an S3 bucket — such as **object creation, deletion, or restoration**.
They’re a cornerstone of **event-driven architectures**, enabling integrations with **AWS Lambda**, **SQS**, or **SNS** without polling S3 manually.
...
### 🧩 Example 1: Trigger AWS Lambda on File Upload (AWS CLI)
...
✅ Trigger: Every time a `.jpg` file is uploaded → Lambda runs.

---

### 🧩 Example 2: Send to SQS Queue
...
✅ Trigger: On delete → message delivered to `process-queue`.
...
### ⚙️ Example Event JSON (Payload to Lambda or SQS)
...
### ⚙️ S3 Event Destinations (Summary)
...
Whenever an object is **created, deleted, or modified**, S3 can trigger **Lambda, SQS, or SNS** — enabling **serverless, event-driven workflows** without polling or manual intervention.
...
AWS provides multiple layers of protection — such as **Versioning**, **MFA Delete**, **Object Lock**, and **access controls** — to ensure data is **recoverable and deletion is intentional**.
...
### 🧩 1️⃣ Enable Versioning (First Line of Defense)

Versioning keeps **every version** of an object — even deleted ones — so you can **restore previous versions** easily.
...
✅ When a user deletes an object, S3 adds a **delete marker** but retains the older version.
...
> 🔒 **Best practice:** Enable Versioning for all production or compliance-critical buckets.

---

### 🧩 2️⃣ Enable MFA Delete (Strong Protection)
...
✅ Protects against accidental or unauthorized object deletion (requires physical MFA token).

> ⚠️ **Note:** Can only be enabled via the **root account** and **AWS CLI**.

---

### 🧩 3️⃣ Use S3 Object Lock (WORM Protection)

Enforces **Write-Once-Read-Many (WORM)** — prevents modification or deletion of objects for a defined **retention period**.
...
✅ Ideal for compliance, audit, or legal-hold data.

---

### 🧩 4️⃣ Restrict Delete Permissions in IAM Policies

Use **least privilege** — deny `s3:DeleteObject` or allow deletes only for admins.
...
✅ Prevents accidental deletions by users or automation roles.

---

### 🧩 5️⃣ Enable S3 Access Logging or CloudTrail
...
✅ Records every `DeleteObject` call with IAM user, time, and IP.

---

### 🧩 6️⃣ Enable S3 Lifecycle Rules (for Safety Deletes)
...
✅ Ensures files are kept for a year before final removal.
...
| ---------------------- | ----------------------------------- | -------------------- |
| **Versioning**         | Keeps deleted file versions         | 🟢 High              |
| **MFA Delete**         | Requires MFA for delete actions     | 🔵 Very High         |
| **Object Lock**        | Makes objects immutable             | 🔴 Maximum           |
| **IAM Restrictions**   | Prevent unauthorized delete actions | 🟢 High              |
| **CloudTrail Logging** | Tracks delete events                | 🟡 Monitoring only   |
| **Lifecycle Rules**    | Safely expire objects               | 🟢 Medium            |
...
2️⃣ Add **MFA Delete**,
3️⃣ Use **Object Lock** for immutability,
4️⃣ Restrict IAM delete permissions,
...
Instead of using the **bucket’s single global endpoint**, access points provide **individual network endpoints** with **dedicated access policies**, simplifying **permission management** and **VPC access control** — especially in **multi-tenant or large data lake** environments.
...
### 🧩 Example: Create an Access Point (AWS CLI)
...
✅ Creates an access point `marketing-ap` linked to `company-analytics`.

---

### 🧩 Example: Attach Access Point Policy
...
✅ Grants read-only access to objects under the `marketing/` prefix.

---

### 🧩 Example: VPC-Only Access Point (Private Access)
...
✅ Access only allowed from the specified VPC — ideal for internal analytics or ETL jobs.
...
It provides **low-latency, high-speed data transfer** over long distances — ideal for users or applications uploading to S3 from **remote regions**.
...
2. It’s then routed over **AWS’s private backbone network** to your S3 bucket’s region.
...
### 🧩 Example: Enable S3 Transfer Acceleration (AWS CLI)
...
### 🧩 Example: Accelerated Endpoint URL
...
✅ Works globally — S3 automatically determines the **nearest edge location**.

---

### 🧩 Example: Upload via Accelerated URL
...
| **Use Case**           | **Example**                                            |
| ---------------------- | ------------------------------------------------------ |
| 🌍 **Global uploads**  | Users around the world uploading to one S3 bucket.     |
| 📸 **Mobile apps**     | Fast media uploads to a centralized S3 region.         |
| 💾 **Data migration**  | Moving TBs of data from remote offices to AWS.         |
| 🎥 **Media ingestion** | Uploading large video files from multiple geographies. |

---

### 💡 In short

**S3 Transfer Acceleration** = **faster, global file transfers** using AWS’s private network via CloudFront edge locations.
✅ Ideal for **long-distance, large, or frequent uploads/downloads** — just enable it and use the accelerated endpoint:
...
To keep traffic **private** — without traversing the public internet — AWS provides **VPC Endpoints for S3** using **PrivateLink**.
This ensures all S3 traffic stays **within the AWS network** — secure, faster, and compliant.
...
✅ Type: **Gateway Endpoint** (specific to S3 and DynamoDB).
This automatically adds a **route** to S3 in your VPC route table.

---

#### 2️⃣ Update Route Table (If Needed)
...
#### 3️⃣ Configure Bucket Policy to Allow Access Only from the Endpoint
...
✅ Ensures only traffic **through the VPC endpoint** can access the bucket.

---

#### 4️⃣ Test Access (From EC2 in Same VPC)

```bash
aws s3 ls s3://my-private-bucket --region ap-south-1
```

✅ Works without Internet Gateway or NAT.

---

### 📋 Types of S3 VPC Access
...
✅ Restrict bucket access with `aws:SourceVpce`.
...
3. **If Cached (Hit):** Content is served instantly from the edge — low latency.
...
### 🧩 Example: Basic CloudFront Setup (S3 Origin)

#### Step 1️⃣ — Create a CloudFront Distribution
...
#### Step 2️⃣ — Access via Distribution Domain

```
https://d123abcde1234.cloudfront.net/index.html
```

✅ Your content is now globally distributed and cached for faster delivery.
...
### 🧩 Example: Restricting Access to S3 Origin (via OAC)
...
✅ Use **Origin Access Control (OAC)** to ensure your S3 bucket is **private** and only accessible through CloudFront.
...
**Amazon CloudFront** = AWS’s **global CDN** for fast, secure content delivery.
...
> 🧩 Think of **Distribution** as: “How CloudFront delivers and secures my content.”

---

### 🧩 Example: CloudFront Architecture
...
### 🧩 Example: AWS CLI (Create a Distribution with an S3 Origin)
...
✅ CloudFront creates a **Distribution** (global endpoint) with your **S3 bucket as the origin**.
...
| **Example**        | S3, ALB, EC2, custom HTTP server.                 | CloudFront’s global edge network endpoint.      |
...
It’s where **end-user requests terminate** — providing **low latency**, **high transfer speed**, and **faster response times** for static and dynamic content delivery.
...
➡️ Result: Faster delivery — data doesn’t travel across the globe every time.
...
| 🧠 **Programmability**      | Supports **Lambda@Edge** and **CloudFront Functions** to execute code near users. |

---

### 🧩 Example: Caching Behavior in Edge Locations
...
```
Client → Edge Location → Served instantly from cache
```

---

### 🧩 Example: Lambda@Edge (Running Code at Edge)
...
✅ Runs at the **Edge Location** — before the request reaches your origin.
...
| **Regional Edge Cache (REC)** | Larger cache layer between origins and edge locations — reduces origin load. |
| **Origin**                    | The actual source (S3, ALB, EC2, etc.) where data is stored.                 |

---

### 🧩 Example: Request Routing Hierarchy
...
Nearest Edge Location (Check Cache)
   ↓
If Miss → Regional Edge Cache (Secondary Layer)
   ↓
If Still Miss → Origin (S3/ALB)
```

✅ Improves cache hit ratio and reduces origin load dramatically.
...
It’s a crucial caching parameter that controls **cache freshness**, **performance**, and **origin load**.
...
2. If **cached and valid (TTL not expired)** → CloudFront serves it instantly.
3. If **expired or not cached** → CloudFront fetches a fresh copy from the **origin** and resets the TTL timer.
...
> 🧠 **Default TTL applies only when neither Cache-Control nor Expires headers are present.**

---

### 🧩 Example: CloudFront Cache Flow

```
User Request → Edge Location Cache Check
   │
   ├── Cache Hit → Served immediately (within TTL)
   └── Cache Miss or TTL Expired → Fetch from Origin → Cache New Copy
```

---

### 🧩 Example: Setting TTLs in Cache Behavior (Terraform)
...
✅ CloudFront will cache objects for **1 hour** unless overridden by origin headers.

---

### 🧩 Example: Origin Cache Headers (S3 / Web Server)
...
→ Tells CloudFront (and browsers) to cache the object for **1 hour**.
...
→ Tells CloudFront until **specific date/time**.

> 🧩 If both headers exist, **Cache-Control** takes precedence.

---

### 📋 CloudFront Cache-Control Logic
...
| ------------------------------- | ----------------- | ------------------------------------- |
| Static Assets (Images, JS, CSS) | 1–24 hours        | Cache aggressively for performance    |
| API / Dynamic Pages             | 0–60 seconds      | Ensure freshness                      |
| HTML Pages                      | 300–600 seconds   | Balance between latency and freshness |
...
✅ **Short TTL** → fresh data, more origin hits.
...
| **OAC (Origin Access Control)**  | CloudFront **signs each request** to your S3 origin using **AWS SigV4 authentication**. Your S3 bucket policy trusts CloudFront’s signed requests instead of an OAI identity — supporting **enhanced security**, **KMS encryption**, and **modern IAM conditions**. |
...
> ⚠️ OAI uses an **IAM-style identity ARN**, which is less flexible and doesn’t support advanced security features like SSE-KMS.

---

### 🧩 Example: **OAC Setup (Recommended)**
...
#### 2️⃣ Attach to CloudFront Distribution
...
✅ CloudFront now **signs every request** using SigV4, and your bucket trusts only your specific distribution.
...
It provides **encryption, DDoS protection, traffic filtering, and access control** — ensuring your content is **delivered securely, reliably, and only to authorized users**.
...
2. CloudFront enforces **security policies** — TLS, WAF rules, geo restrictions, signed URLs, etc.
...
### 🧩 Key Security Features in CloudFront

| **Category**                              | **Feature**                        | **Purpose / Protection**                                                  |
| ----------------------------------------- | ---------------------------------- | ------------------------------------------------------------------------- |
| 🔒 **Encryption**                         | **HTTPS (TLS 1.2/1.3)**            | Encrypts data in transit between clients ↔ edge ↔ origin.                 |
...
| 🧠 **Lambda@Edge / CloudFront Functions** | **Custom Security Logic**          | Run code to inspect, validate, or modify requests in real time.           |
| 🧾 **Access Logging**                     | **Real-time & Standard Logs**      | Logs every viewer and edge request for audit & compliance.                |

---

### 🧩 Example 1: Enforce HTTPS for All Viewers
...
✅ Redirects all HTTP requests to HTTPS → ensures secure connections.

---

### 🧩 Example 2: Restrict Direct S3 Access via **OAC**
...
✅ Only CloudFront (signed SigV4 requests) can access your bucket — no direct public access.

---

### 🧩 Example 3: Enable AWS WAF with CloudFront
...
✅ Blocks malicious IPs, bots, and injection attacks at the edge — before they reach your origin.

---

### 🧩 Example 4: Use Signed URLs for Private Content
...
✅ Only authenticated users with valid signatures can access protected files.

---

### 🧩 Example 5: Geo Restriction (Block Specific Countries)
...
✅ Denies access from listed countries at edge locations.

---

### 📊 How CloudFront Protects Your Origin
...
✅ Encrypts data (TLS),
✅ Protects origins (OAC),
✅ Filters malicious traffic (WAF + Shield), and
✅ Controls access (Signed URLs, Geo restrictions).
...
**Signed URLs** and **Signed Cookies** in **Amazon CloudFront** are **access control mechanisms** that restrict who can access your **private content** — such as videos, files, or API responses — served through CloudFront distributions.
...
### 🧩 Types of Signed Access

| **Type**          | **Use Case**                                    | **Example**                                         |
| ----------------- | ----------------------------------------------- | --------------------------------------------------- |
| **Signed URL**    | Best for single files (e.g., a video or PDF).   | Pre-sign individual URLs per user.                  |
| **Signed Cookie** | Best for multiple files (e.g., web app assets). | Set once → access multiple resources within domain. |
...
[User Request] → CloudFront Distribution
       │
       ├── Signed URL validation (policy, expiry, signature)
       │
       └── If valid → Serve content from cache or origin
```

---

### 🧩 Example 1: Generate a Signed URL (AWS CLI / Python)
...
✅ Valid for 1 hour — after that, access is denied.

---

### 🧩 Example 2: Signed Cookie (For Multiple Objects)
...
✅ All requests with this cookie can access matching objects under the policy (e.g., `/videos/*`).

---

### 📋 Example Custom Policy (Fine-Grained Control)
...
### 📊 Comparison — Signed URLs vs Signed Cookies
...
✅ **Signed URL** → best for single objects (e.g., one file).
...
It ensures **new or updated content** (from your origin — e.g., S3, ALB, or EC2) is delivered **immediately** to users, instead of serving outdated cached versions.
...
If you update your content at the origin, CloudFront **still serves old versions** until TTL expiry — unless you **invalidate** them.
...
```
User Request → CloudFront Edge (cached version)
      │
      ├── Invalidation issued (object marked stale)
      │
      └── Next request → Fetch new object from Origin (S3/ALB)
```

---

### 🧩 Example 1: Invalidate via AWS CLI
...
✅ Forces CloudFront to refresh **all cached content**.

---

### 🧩 Example 2: Invalidate via AWS Console

1. Go to **AWS CloudFront → Distributions**.
2. Select your distribution → **Invalidations → Create Invalidation**.
3. Enter path(s) (e.g., `/index.html` or `/*`).
4. Click **Invalidate**.

✅ You’ll see an **Invalidation ID** and its **status** (`InProgress → Completed`).

---

### 🧩 Example 3: Terraform Example
...
### 🧩 Example: Automated Invalidation in CI/CD (Jenkins / GitHub Actions)
...
✅ Ensures your deployment pipeline always serves **latest app assets** (HTML, CSS, JS).

---

### 📊 Invalidation Cost & Limits
...
| **Time to Propagate** | Usually completes in **1–2 minutes** globally.                   |
...
✅ Trigger via CLI, Console, or CI/CD tools using:
...
**Amazon CloudFront** allows you to **control access based on the viewer’s country** using **Geo Restriction (Geo Blocking)**.
This feature lets you **block or allow** users from specific countries from accessing your content — all handled at **CloudFront edge locations**, before requests even reach your origin (like S3, ALB, or EC2).

It’s ideal for **regional compliance**, **licensing restrictions**, or **content distribution control** (e.g., GDPR, streaming rights).
...
### 🧩 Example 1: Restrict via AWS Console

1. Go to **CloudFront → Distributions**.
2. Choose your distribution → **Behaviors → Edit Behavior**.
3. Scroll to **Geo Restriction (Viewer Location)**.
4. Select **Blacklist** or **Whitelist**.
5. Add country codes (e.g., `CN`, `RU`, `US`, etc.).
6. Save changes.

✅ CloudFront will now block access from the listed countries with a **403 Forbidden** error.
...
✅ Blocks users from **China (CN)** and **Russia (RU)**.
...
✅ Only allows users from **US**, **Canada**, and **UK**.

---

### 🧩 Example 4: Custom Geo Restriction Logic (Lambda@Edge)
...
✅ More flexible — supports custom redirections, page variations, or analytics based on country.
...
✅ Provides a user-friendly message like “Content not available in your region.”
...
| **Time to Apply**             | Typically propagates in 5–10 minutes globally.  |
...
**CloudFront Geo Restriction** lets you **allow or block** users from specific countries directly at AWS **edge locations** — no origin traffic involved.
✅ Configure via **console, CLI, or Terraform**.
...
Route 53 acts as the **DNS service**, while CloudFront serves as the **content delivery network (CDN)** — together they enable **global, low-latency content delivery** with **custom domains, SSL/TLS, and intelligent routing**.
...
User → www.example.com
         ...
         ▼
   Route 53 (DNS)
   └── Alias Record → CloudFront (d123abcd.cloudfront.net)
         │
         ▼
...
### 🧩 Example: Integrate CloudFront with Route 53 (Custom Domain Setup)

#### Step 1️⃣ — Create CloudFront Distribution
...
#### Step 2️⃣ — Request an SSL/TLS Certificate in ACM

Use **AWS Certificate Manager (ACM)** (in `us-east-1` — mandatory for CloudFront):
...
✅ Validate via DNS record Route 53 provides.

---

#### Step 3️⃣ — Attach Custom Domain & SSL Cert to CloudFront
...
✅ CloudFront now serves content using your **custom domain** with HTTPS.

---

#### Step 4️⃣ — Create Route 53 Alias Record
...
✅ `Z2FDTNDATAQYW2` is the **CloudFront hosted zone ID (global, same for all regions)**.
...
### 📋 Benefits of Route 53 + CloudFront Integration
...
✅ Route 53 maps your **custom domain** (e.g., `cdn.example.com`) to your **CloudFront distribution**, using **Alias records** and **ACM certificates** for HTTPS.
...
### 📋 Table — Key Config Parameters & Recommended Values
...
| Cache TTLs       | `default` 1h; static assets long (1d–30d) with versioning | Performance + safe invalidation strategy            |
...
Build your React app → upload to a **private S3** bucket, serve via **CloudFront + OAC + ACM (HTTPS)**, add edge security headers (CloudFront Function), attach **WAF**, and automate deploys + invalidations in CI/CD.
...
2. **Inspect CloudFront cache behavior & TTLs**
   ...
   Check `DefaultCacheBehavior` / `CacheBehaviors` → `MinTTL`, `DefaultTTL`, `MaxTTL`, `ForwardedValues` or `CachePolicyId`.
...
5. **Check CI/CD deploy logic** — confirm it uploads new files with correct names/headers and triggers invalidation or uses versioned filenames.

6. **Browser & Service Worker** — test in incognito, disable service worker, or clear cache to rule out client caching.
...
### 🧩 Triage checklist — run these in order (fast wins first)
...
6. **If using OAI (legacy) — verify OAI ARN in bucket policy**
...
| `403` and `KMS Access Denied` in CloudTrail      |                   SSE-KMS key policy doesn’t include CloudFront | Add CloudFront service principal to KMS key policy (allow Decrypt/GenerateDataKey) with `SourceArn` condition |
...
| `403` after migrating OAI→OAC                    |      Old bucket policy references OAI but distribution uses OAC | Update bucket policy to OAC-style condition (cloudfront service principal & SourceArn)                        |
...
| 📦 No compression             | Larger data over wire       | Compress before upload (gzip, zip)                                   |

---

### 🧩 Solution 1️⃣ — Enable **S3 Transfer Acceleration**
...
✅ Automatically routes upload via nearest AWS edge → internal AWS backbone → target region.

📊 Typical speed gains: **2×–10× faster** for uploads from Europe/US → Asia-Pacific regions.

---

### 🧩 Solution 2️⃣ — Use **Multipart Uploads** (for large files >100 MB)
...
✅ Parallel parts = faster throughput (default: 10 parallel threads in AWS CLI).

---

### 🧩 Solution 3️⃣ — Tune AWS CLI upload concurrency
...
✅ More threads = faster parallel upload (tune based on bandwidth).

---

### 🧩 Solution 4️⃣ — Use **S3 Multi-Region Access Points (MRAP)**
...
✅ Automatically routes uploads to nearest healthy region, replicates via CRR.

---

### 🧩 Solution 5️⃣ — Compress before upload

```bash
tar -czf data.tar.gz ./data && aws s3 cp data.tar.gz s3://mybucket/
```

✅ Reduces transfer size and latency, especially over slow WAN.

---

### 🧩 Solution 6️⃣ — Check network path & throughput
...
✅ Identify high RTT or packet loss → use nearer region or TA.
...
✅ Fix: **Enable Transfer Acceleration**, **use multipart uploads**, **tune concurrency**, or **upload to the nearest region**.
...
Recovery depends on your **S3 bucket configuration** — particularly if you had **Versioning**, **MFA Delete**, or **Backup (Replication/Glacier)** enabled.
If versioning was on — your data is **not truly gone**; you can restore it easily.
...
#### ✅ **Case 1: Versioning Enabled (Best Case — Recoverable)**
...
✅ Your object is now restored and visible again in S3.

---

#### ⚠️ **Case 2: No Versioning (Files Permanently Deleted)**
...
#### 🧩 Example: Restore via AWS Console (Versioned Bucket)
...
✅ Requires MFA token to permanently delete versions — strong protection against scripts or human errors.

---

#### 🧩 Example: CloudTrail Audit Log (Who Deleted It)
...
✅ Shows which IAM user/role or automation pipeline executed the delete.
...
| **Area**                | **Recommendation**                                                 |
| ----------------------- | ------------------------------------------------------------------ |
| 🧾 **Versioning**       | Always enable for all critical buckets (`Status=Enabled`).         |
...
If **versioning** was enabled → ✅ you can restore files instantly by removing the delete marker or copying an older version.
...
4. **OAC (Origin Access Control)** signs CloudFront → S3 requests with AWS SigV4, proving CloudFront’s identity.
5. The S3 bucket policy **trusts only your CloudFront distribution ARN** — rejecting all other traffic.
...
#### **2️⃣ Create an Origin Access Control (OAC)**
...
✅ CloudFront becomes your public endpoint (e.g., `https://d123abcd.cloudfront.net`).

---

#### **4️⃣ Apply Secure S3 Bucket Policy**
...
✅ This ensures **only CloudFront** (with that specific distribution ARN) can read from S3.

---

#### **5️⃣ Add SSL Certificate (ACM) for HTTPS**
...
#### **6️⃣ Add Custom Domain via Route 53**
...
✅ Now `https://www.example.com` securely serves content through CloudFront.
...
| Route53 Alias       | → CloudFront domain                      | Custom domain mapping             |
...
Hosting a static site on **S3 + CloudFront** is cost-efficient by default — but unnecessary requests, long invalidations, and poor cache or storage design can quietly inflate bills.
...
### 🧩 1️⃣ — **Optimize S3 Storage & Access**

| **Action**                      | **Command / Example**                                                                                                                     | **Benefit**                                   |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| **Enable Lifecycle Policies**   | Move older assets to cheaper tiers: <br>`hcl<br>rule { id = "expire-old-assets" transition { days = 30 storage_class = "STANDARD_IA" } }` | 40–70% storage cost reduction                 |
...
| **Use versioned file naming**   | `app.v123.js` → allows long cache TTLs, fewer requests                                                                                    | Fewer reuploads & invalidations               |
| **Disable unnecessary logging** | Keep only CloudFront logs; disable per-object access logs unless needed                                                                   | Reduces S3 PUT costs                          |

---

### 🧩 2️⃣ — **Tune CloudFront Caching for Fewer Origin Hits**
...
| **Use Cache Policies**          | Custom CloudFront Cache Policy → control headers/query | Avoids unnecessary cache misses                              |
...
### 🧩 3️⃣ — **Reduce CloudFront Data Transfer Costs**
...
### 🧩 4️⃣ — **Automate Smart Deployment & Cache Invalidation**
...
✅ Avoids “`/*`” invalidations for large sites (saves up to 90% per deploy).

---

### 🧩 5️⃣ — **Use S3 Transfer Acceleration Judiciously**

Only enable **S3 Transfer Acceleration** for **global uploads** (e.g., multi-region CI/CD).
If users are mostly local to the S3 region → **disable TA** to save transfer fees.
...
### 🧩 6️⃣ — **Monitor & Alert on Cost Drivers**
...
### 🧩 7️⃣ — **Leverage Free Tier and Regional Pricing**
...
👉 Keep S3 + CloudFront in same region to avoid **cross-region data transfer charges**.
Avoid mixing `S3 (ap-south-1)` with CloudFront origin in `us-east-1` unless required.

---

### 🧩 8️⃣ — **Consider AWS Graviton CI/CD Builders**
...
| **Area**      | **Recommendation**                                        |
| ------------- | --------------------------------------------------------- |
| 💾 Storage    | Enable S3 Lifecycle rules (IA/Glacier) for old builds     |
| 🌎 Delivery   | Use CloudFront caching & regional edge caches             |
| 🔄 Deploys    | Versioned filenames + targeted invalidations              |
| 🔒 Security   | Keep S3 private (OAC) — no cost impact but protects data  |
| 📊 Monitoring | Enable Cost Explorer + CloudWatch metrics                 |
| 🧠 Automation | Automate lifecycle + invalidation in CI/CD                |
| 🧾 Review     | Use AWS Trusted Advisor → “Cost Optimization” tab monthly |
...
### 🧩 Triage checklist (fast, in order)
...
### 🧩 Examples / Commands / CI snippets

#### GitHub Actions — best-practice snippet (OIDC role assumption)
...
#### 3) Minimal secure S3 bucket policy — **allow only CloudFront distribution**
...
> Note: Deny statements override Allows — test in staging first.
...
| **Deny if not SourceVpce**           | Private uploads from VPC only   | ✅ For internal-only access                        |
| **Remove static website endpoint**   | Avoid public website origin     | ✅ Necessary to enable private origin              |
| **S3 Access Point (VPC restricted)** | Complex multi-tenant setups     | ✅ Fine-grained, VPC-only access                   |
| **WAF / CloudFront rules**           | Extra layer (rate-limit, block) | ✅ Prevents abusive traffic but not direct S3 hits |
...
Optimizing cache design — using **versioned assets, cache-control headers, and selective invalidation** — nearly eliminates the need for manual invalidations.
...
### 🧩 Common Causes of Costly Invalidations
...
### 🧩 1️⃣ — **Use Versioned File Names (Best Practice)**

Instead of invalidating `/main.js` → upload `/main.v1.js`, `/main.v2.js`.
...
✅ **No invalidation needed** — old caches expire naturally while new files have unique names.

---

### 🧩 2️⃣ — **Set Smart Cache-Control Headers**
...
✅ No need to invalidate on every deploy — HTML fetches latest asset list.

---

### 🧩 3️⃣ — **Invalidate Only Critical Paths**
...
### 🧩 4️⃣ — **Increase TTLs for Static Assets**
...
✅ Combine this with **immutable versioned files** for best performance & cost balance.

---

### 🧩 5️⃣ — **Use Separate Cache Behaviors for Dynamic vs Static**
...
✅ Only short-cache HTML; let assets live long at edge.

---

### 🧩 6️⃣ — **Bundle Invalidations per Deploy**

Multiple paths → one invalidation:
...
✅ Cost = 1 invalidation request (multiple paths allowed).

---

### 📋 Cost Table (Example Estimate)

| **Scenario**                               | **Invalidations / Month** | **Paths per Invalidation** | **Approx. Cost**                  |
| ------------------------------------------ | ------------------------- | -------------------------- | --------------------------------- |
| Every deploy uses `/*`                     | 20                        | 1                          | $0 (first 1k) → $0.005 thereafter |
...
It combines **Amazon S3** (as origin storage) with **Amazon CloudFront** (as global CDN) — secured via **Origin Access Control (OAC)**, **ACM-managed HTTPS**, and **AWS WAF**.
...
| 🌎 **Edge Locations**                 | AWS PoPs (Points of Presence) that cache content close to users for faster delivery.                        |
| 🔑 **OAC / OAI**                      | Authenticates CloudFront requests to S3; ensures users can’t bypass CloudFront to access S3 directly.       |
| 🔐 **ACM Certificate**                | Provides SSL/TLS encryption for CloudFront (HTTPS traffic). Managed automatically by AWS.                   |
| 🧭 **Route 53 Alias Record**          | Maps a custom domain (e.g., `cdn.myapp.com`) to the CloudFront distribution using a zero-cost alias record. |
| 🧱 **WAF (Web Application Firewall)** | Protects against OWASP Top 10 threats, DDoS, bots, and malicious IPs at the edge.                           |
...
| **Monitoring**        | Enable **CloudFront access logs** → S3; use **CloudWatch metrics** (4xx/5xx, CacheHitRate, BytesDownloaded).                 |
